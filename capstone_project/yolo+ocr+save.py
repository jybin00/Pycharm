...
font3 = ImageFont.truetype('/Library/Fonts/a고딕16.otf', 60)
crop_img2 = np.zeros((100, 200, 3))
...
while True:
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            if int(time.time()) % 1 is 0:
                if x is not 0:
                    crop_img = frame[y:y + dh, x:x + dw]
                    crop_img2 = crop_img.copy()
                    try:
                        crop_img2 = cv2.resize(crop_img2, (400, 200))
                        h, w, c = crop_img2.shape
                        frame[200:200+h, 25:25+w] = crop_img2
            ...
            try:
                ...
                draw.text((x + 160, y - 30), ' ' + output_words[i], font=font2, fill=(200, 255, 155))
                draw.text((10, 50), ' ' + 'Detected character :', font=font2, fill=(200, 255, 155))
                draw.text((10,100), ' ' + output_words[i], font=font3, fill=(200, 255, 155))
            except IndexError:
                pass
